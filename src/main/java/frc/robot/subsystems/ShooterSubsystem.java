...
public class ShooterSubsystem extends SubsystemBase {
    ...
    private AngularVelocity calculateShooterSpeedFromRobotDistance() {
        double d = hypotenuseToAllianceHub.in(Units.Meters);
        double g = Constants.g.in(Units.MetersPerSecondPerSecond);
        double v = Math.sqrt(
            ( d * g )
            /
            Math.sin( 2.0 * Constants.Shooter.LAUNCH_ANGLE.in(Units.Radians) )
        ); // projectile motion range equation
        AngularVelocity omega = Units.RadiansPerSecond.of(v / Constants.Shooter.FLYWHEEL_DIAMETER.div(2.0).in(Units.Meters)); // v/r
