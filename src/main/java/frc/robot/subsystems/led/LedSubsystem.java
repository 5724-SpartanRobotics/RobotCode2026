...
public class LedSubsystem extends NopSubsystemBase {
	...
	private enum BaseState {
		RAINBOW, ACTIVE_OFF, DISABLED_STANDBY
	}

	private BaseState _baseState = BaseState.RAINBOW;

	private LedIO.LedIOInputs inputs = new LedIO.LedIOInputs();
	...
	private LedSubsystem() {
		...
		// The magic of floating point numbers does something to this value, but I'm not
		// sure
		// exactly what. This is calculated somehow, and this calculation yields correct
		// results,
		// but I'm not exactly sure how I came to it.
		final double countCoeff = WPILIB_BITS_PER_PIXEL / LedConstants.STRIP_BITS_PER_PIXEL;
		// This magic value of 9 is to reduce the length of the buffer so we aren't
		// trying to index
		// too many LEDs out of the strip. Not calculated, I just counted on the actual
		// strip.
		int adjustedStripLength = (int) (LedConstants.COUNT * countCoeff) - 9;
		m_buffer = new AddressableLEDBuffer(adjustedStripLength);
		m_led.setLength(adjustedStripLength);
		m_led.start();

		if (RobotMode.is(RobotMode.Simulation)) {
	...
	public void periodic() {
		...
		if (dsEnabled)
			_hasEverEnabled = true;
		if (dsDisabled && !_lastDisabled)
			_disabledStartTimeSec = now;
		...
		if (!_hasEverEnabled || !dsAttached)
			_baseState = BaseState.RAINBOW;
		else
			if (dsEnabled)
				_baseState = BaseState.ACTIVE_OFF;
			else {
				if (now - _disabledStartTimeSec > kDefaultStandbyDelay.in(Units.Seconds))
					_baseState = BaseState.DISABLED_STANDBY;
				else
					_baseState = BaseState.ACTIVE_OFF;
			}
	...
	public void log() {
		inputs.hasEverEnabled = _hasEverEnabled;
		inputs.baseState = _baseState.toString();

		inputs.notificationCount = _notifications.size();
		inputs.topNotificationColor = _notifications.isEmpty()
			? "None"
			: _notifications.peek().color.toString();

		inputs.timeSinceDisabled = Timer.getFPGATimestamp() - _disabledStartTimeSec;

		inputs.rainbowOffset = _rainbowOffset;

		inputs.currentColorRGB = new double[]{
			_currentRenderedColor.red,
			_currentRenderedColor.green,
			_currentRenderedColor.blue
		};

		inputs.currentColorHex = String.format(
			"#%02X%02X%02X",
		...
		// Flatten LED strip
		double[] strip = new double[m_buffer.getLength() * 3];
